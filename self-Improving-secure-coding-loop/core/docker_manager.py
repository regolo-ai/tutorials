"""Docker Service Manager for Closed-Loop Secure Coding Agent.
Manages required background services (Qdrant vector engine, Cognee knowledge graph backend).
Features:
...
REQUIRED_SERVICES: Dict[str, Dict[str, Any]] = {
    "qdrant": {
        ...
        "container_name": "closed-loop-qdrant",
        "image": "qdrant/qdrant:latest",
        "default_port": 6333,
        "container_port": 6333,
        "description": "High-performance vector database for Cognee memory & code embeddings",
        "health_path": "/healthz",
    },
    "cognee": {
        "service_key": "cognee",
        "name": "Cognee Knowledge Graph Backend",
        "container_name": "closed-loop-cognee",
        "image": "cognee/cognee:main",
        "default_port": 8800,
        "container_port": 8000,
        "description": "Self-hosted knowledge graph, relational memory & tenant isolation engine",
        "health_path": "/health",
    },
}
...
def find_available_port(start_port: int, max_attempts: int = 100, host: str = "127.0.0.1") -> int:
    ...
    while curr < start_port + max_attempts:
        if not is_port_in_use(curr, host=host):
            # Double check by attempting to bind to the port
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                    s.bind((host, curr))
                    return curr
